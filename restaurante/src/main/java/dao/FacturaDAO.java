...
public class FacturaDAO {
    private static final String SQL_INSERTAR = "INSERT into factura (mesa) values(?)";
    ...
    private static final String SQL_SELECT_FACTURA = "select * from factura F inner join facturaProductos fp "
            + "on F.id = fp.factura inner join productos p on fp.producto = p.id where F.id = ? ";
    
    public static int insertar(int mesa){
    ...
    public static int seleccionarId(int mesa){
        int id = 0;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        Connection conn = null;
        try {
            conn = Conexion.getConnection();
            stmt = conn.prepareStatement(SQL_SELECCIONAR_FACTURA_POR_MESA_NO_PAGADA);
            stmt.setInt(1, mesa);
